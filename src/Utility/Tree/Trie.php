...
class Trie
{
     ...
     * @return $this|bool
     */
    public function append($index_str, $data = [], $is_py = false, $chinese = '')
    {
        $str     = trim($index_str);
     ...
     * @param bool   $delete_tree
     * @param bool   $is_py
     * @param string $chinese
     *
     * @return bool
     */
    public function delete($index_str, $delete_tree = false, $is_py = false, $chinese = "")
    {
        $str     = trim($index_str);
        $chinese = trim($chinese);
        if ($is_py && empty($chinese)) {
...
//        var_dump($this->nodeTree['e59bbd']);exit();
     ...
     * @return mixed
     */
    private function &_appendWordToTree(&$tree, $code, $word, $end = false, $data = [], $full_str = '', $is_py = false)
    {
        if (!isset($tree[$code])) {
            $tree[$code] = [
                'end'   => $end,
