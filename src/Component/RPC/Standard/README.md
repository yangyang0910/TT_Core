...
class Rpc extends \Core\Component\RPC\Standard\AbstractInterface\AbstractService
{
    function test()
    {
    ...
    function test2()
    {
        $this->getResponse()->setStatus(\Core\Component\RPC\Standard\Bean\Response::STATUS_RESPONSE_DETACH);
    }
}

class Rpc2 extends \Core\Component\RPC\Standard\AbstractInterface\AbstractService
{
    function fuck()
    {
...
$conf = new \Core\Component\RPC\Standard\Config();
...
$trigger = new \Core\Component\Error\Trigger();
$rpc = new \Core\Component\RPC\Standard\Rpc($conf,$trigger);
try{
...
$trigger = new \Core\Component\Error\Trigger();
$rpc = new \Core\Component\RPC\Standard\Rpc(new \Core\Component\RPC\Standard\Config(),$trigger);

go(function ()use($rpc){
    try{
        $client = $rpc->client();
        //模拟在未开启服务广播发现的时候，注册已知节点
        $node = new \Core\Component\RPC\Standard\Bean\ServiceNode();
        ...
        $client->addCall('a','test')
            ->success(function (\Core\Component\RPC\Standard\Bean\Response $response){
                var_dump('success');
            })->fail(function (){
                var_dump('fail');
            });
        $client->addCall('a','test2')
            ->success(function (){
                var_dump('success2');
            })->fail(function (\Core\Component\RPC\Standard\Bean\Response $response){
