...
use Core\Component\Pool\Exception\PoolNumError;
...
abstract class AbstractPool
{
    ...
    public function getObj($timeout = null, $beforeUseTryTimes = 3)
    {
        if ($timeout === null) {
     ...
     */
    public function gcObject($idleTime)
    {
        $list = [];
        while (true) {
            if (!$this->poolChannel->isEmpty()) {
    ...
    public function keepMin($num = null)
    {
        if ($num == null) {
    ...
    */
    public function preLoad($num = null)
    {
     ...
     * 把一个对象归还到队列中
     */
    protected function putObject($object)
    {
        if (is_object($object)) {
